...
<body>
	...
	if(isset($_POST['submit'])){
		...
		$userid = $_SESSION['ID'];
		$query2 = mysqli_query("SELECT * FROM member WHERE memberid = '$userid'") or die (mysqli_error());
		$user = mysqli_fetch_array($query2);
		$biddername = $user['memberid'];
		$prodname = $prod['prodname'];
		if($bidamount > $high){
		mysqli_query("INSERT INTO bidreport(productid,bidder,biddatetime,bidamount,status) VALUES ('$id','$biddername',now(),'$bidamount',0)");
