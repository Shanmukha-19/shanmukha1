...
<body>
	...
	<?php
		$id = $_GET['id'];
		$pname = mysqli_query("SELECT * FROM products WHERE productid='$id'")or die(mysqli_error());
	...
	?>
        ...
        <tbody>
        	<?php 
				$prodid = $_GET['id'];
				$query = mysqli_query("SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN products ON products.productid = bidreport.productid WHERE products.productid = '$prodid'") or die(mysql_error());
