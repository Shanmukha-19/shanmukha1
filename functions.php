<?php
	require ("db.php");
function cats(){
		$query = mysqli_query("SELECT * FROM `pcategories`") or die (mysqli_error());
...
//shows the products on watch--------
 function onwatch(){
		...
		$query1 = mysqli_query("SELECT * FROM watchlist WHERE memberid = '$who_u'");
		while($row1 = mysqli_fetch_array($query1)){
			$prod = $row1['productid'];
			$query = mysqli_query("SELECT * FROM products WHERE productid = '$prod'");
			while($row = mysqli_fetch_array($query)){
				echo "<div class='prod_box'>";
					echo "<div class='top_prod_box'></div>";
					echo "<div class='center_prod_box'>";
...
function logform(){
	...
	echo '<div class="title_box">Welcome</div>
		...
		$query = mysqli_query("SELECT * FROM member WHERE memberid = '$hisid' ");
